...
The configuration file, C<krang_addon.conf> contains the name and
version of the addon:

  Name Turbo
  Version 1.00

The file structure in an addon matches existing directories in Krang.
It is installed into the C<addons/> directory in Krang.  In this case
the structure above would be placed in C<addons/Turbo>.  To run the
C<krang_turbo> script you would run:

  $ addons/Turbo/bin/krang_turbo
...
B<NOTE:> As of Krang 1.102 there is a new script to automate some of
the work here:

  bin/krang_addon_framework



=head2 C<krang_addon.conf>

You must create a C<krang_addon.conf> file in your add-on's directory.
The file follows the same Apache-inspired format as Krang's krang.conf
file.  I'll explain each available directive.
