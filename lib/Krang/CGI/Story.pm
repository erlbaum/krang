...
sub new_story {
    ...
    my %type_labels =
      map { ($_, pkg('ElementLibrary')->top_level(name => $_)->display_name) } @types;
    $template->param(
    ...
    foreach my $story_type (@types) {
        ...
        if (   ($slug_use eq 'require')
            || ($slug_use eq 'encourage')
            || ($slug_use eq 'discourage')
            || ($slug_use eq 'prohibit'))
        {
            push @slug_entry_by_type,
              {
