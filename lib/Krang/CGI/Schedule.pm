...
sub list_all_row_handler {
    ...
    if ($frequency eq 'One Time') {
        $s_params =
          Time::Piece->from_mysql_datetime($schedule->next_run)->strftime('%m/%d/%Y %I:%M %p');
    } elsif ($frequency eq 'Hourly') {
        ($schedule->minute eq '0')
          ? ($s_params = 'on the hour')
          : ($s_params = $schedule->minute . " minutes past the hour");
    ...
    $s_params =
      ($frequency eq 'Daily') ? ($frequency . ' at ' . $s_params) : ($frequency . ', ' . $s_params);
