...
my @trashed = pkg('Story')->find(trashed => 1);
...
@trashed = pkg('Story')->find(trashed => 1, ids_only => 1);
is(scalar(@trashed), 0, "No stories trashed (with ids_only option)");

@archived = pkg('Story')->find(archived => 1, ids_only => 1);
