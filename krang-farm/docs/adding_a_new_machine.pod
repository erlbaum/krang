...
(Note: if you need to make an external network connection to get
software then set the interface to DHCP which is needed for VMWare's
NAT system.  Just set it back to $IP when you're done.)
